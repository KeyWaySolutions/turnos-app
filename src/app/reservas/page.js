...
export default function ReservasPage() {
  ...
  // Sync today's date on hydration to avoid SSR mismatch
  useEffect(() => {
  ...
  // Adjust day order (0: Monday, 6: Sunday)
  ...
  const handleSubmitBooking = async (e) => {
    ...
    try {
      ...
      setMessage({
        type: 'error',
        text: err.message || 'Ocurrió un error al procesar tu reserva. Por favor, asegúrate de que la base de datos esté configurada.'
      });
    } finally {
      ...
      <header className="border-b border-brand-teal/10 bg-white py-5 px-6 sm:px-12 flex justify-between items-center shadow-sm select-none">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className="text-xs ml-2 text-brand-teal font-medium border-l border-brand-teal/20 pl-2">Portal de Reservas</span>
          </div>
        </div>
        <a 
          href="/login" 
            ...
            <div className="border border-brand-teal/10 rounded-2xl p-4 sm:p-5 bg-white mb-6">
              ...
              <div className="grid grid-cols-7 gap-1 text-center">
                {calendarDays.map((dayDate, idx) => {
                  ...
                  // Enable weekdays that are not in the past
                  ...
                  if (isSelected) {
                    // Selected day style uses specified luminous mint tone #9BF2C1 and border green #73D97A
                    ...
                    return (
                      <button
                        ...
                        className={`py-2.5 px-3 text-xs font-bold rounded-lg border transition-all text-center cursor-pointer ${
                          isTimeSelected
                            // Selected time uses luminous green #73D97A
              ...
              {message.text && (
                <div className={`p-4 rounded-xl flex items-start gap-3 transition-all ${
                  message.type === 'success'
                    // Luminous tones (#9BF2C1 & #73D97A) for success feedback
